...
# Ensure GradScaler is compatible with MPS and CPU
...
def load_captioning(uploaded_files, concept_sentence):
    ...
    updates.append(gr.update(visible=True))
    updates.append(gr.update(placeholder=f'A portrait of a person in a bustling cafe {concept_sentence}',
...
def start_training(
    ...
    try:
        if whoami()["auth"]["accessToken"]["role"] == "write" or "repo.write" in \
                whoami()["auth"]["accessToken"]["fineGrained"]["scoped"][0]["permissions"]:
            gr.Info(
                f"Starting training locally {whoami()['name']}. Your LoRA will be available locally and on Hugging Face after it finishes.")
        else:
            push_to_hub = False
            gr.Warning(
                "Started training locally. Your LoRA will only be available locally because you didn't login with a `write` token to Hugging Face")
    except:
        push_to_hub = False
        gr.Warning(
            "Started training locally. Your LoRA will only be available locally because you didn't login with a `write` token to Hugging Face")
...
train:
  ...
  optimizer: adamw  # A standard, CPU-compatible optimizer
